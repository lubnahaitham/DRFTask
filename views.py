...
from task.serializers import OredrSerializer, PetSerializer, PriceSerializer
...
def order_list(request):
    ...
    if request.method == 'GET':
        ...
        serializer = OredrSerializer(orders, many=True)
        return JsonResponse(serializer.data, safe=False)

    elif request.method == 'POST':
        data = JSONParser().parse(request)
        serializer = OredrSerializer(data=data)
        if serializer.is_valid():
            serializer.save()
            return JsonResponse(serializer.data, status=201)
        return JsonResponse(serializer.errors, status=400)
...
def order_detail(request, pk):
    ...
    if request.method == 'GET':
        serializer = OredrSerializer(orders)
        return JsonResponse(serializer.data)

    elif request.method == 'PUT':
        data = JSONParser().parse(request)
        serializer = OredrSerializer(orders, data=data)
        if serializer.is_valid():
            serializer.save()
            return JsonResponse(serializer.data)
        return JsonResponse(serializer.errors, status=400)
